document.addEventListener('DOMContentLoaded', () => {
  ...
  const update = () => {
    ...
    if (
      ballRect.intersects(leftPaddleRect) ||
      ballRect.intersects(rightPaddleRect)
    ) {
      ballSpeedX *= -1
    }
  ...
  const onKeyDown = (e) => {
    ...
    if (
      e.key === 'ArrowDown' &&
      rightPaddle.offsetTop + rightPaddle.offsetHeight + paddleSpeed <=
        pong.offsetHeight
    ) {
      rightPaddle.style.top = rightPaddle.offsetTop + paddleSpeed + 'px'
    }
    ...
    if (
      e.key === 's' &&
      leftPaddle.offsetTop + leftPaddle.offsetHeight + paddleSpeed <=
        pong.offsetHeight
    ) {
      leftPaddle.style.top = leftPaddle.offsetTop + paddleSpeed + 'px'
    }
